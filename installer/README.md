# Building the Steady installer

Steady ships as a **self-contained, single-file** Windows app plus a small
...
"C:\Program Files\dotnet\dotnet.exe" publish .\SteadyOverlay.csproj `
...
| Artifact | Path |
| --- | --- |
| Self-contained exe | `bin\publish\win-x64\SteadyOverlay.exe` |
| Installer | `bin\installer\Steady-Setup-1.0.0.exe` |

> Kill any running `SteadyOverlay.exe` before publishing/compiling — the
...
- **No trimming.** WPF is not trim-safe — `PublishTrimmed=true` strips XAML/
  reflection-loaded types and Steady will crash at runtime (it loads
...
$cert = New-SelfSignedCertificate -Type CodeSigningCert `
  -Subject "CN=Steady Test" -CertStoreLocation Cert:\CurrentUser\My
...
& $signtool sign /fd SHA256 /sha1 $cert.Thumbprint /t http://timestamp.digicert.com `
  ".\bin\publish\win-x64\SteadyOverlay.exe"
```
...
- Sign **both** the published `SteadyOverlay.exe` and the final
  `Steady-Setup-*.exe`, always with `/t` (a timestamp URL) so signatures stay
  valid after the cert expires.

## Autostart note

The installer **does not** add its own "start with Windows" entry by default.
Steady's in-app **"Start with Windows"** toggle owns the
`HKCU\...\Run\Steady` value (`"<exe>" --autostart`). The optional installer
